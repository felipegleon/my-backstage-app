# This dockerfile builds an image for the backend-a package.
...
# yarn install
# yarn tsc
# yarn build:backend-a
#
# Once the commands have been run, you can build the image using `yarn build-image`

FROM node:20-bookworm-slim

# Install isolate-vm dependencies, these are needed by the @backstage/plugin-scaffolder-backend-a.
...
# From here on we use the least-privileged `node` user to run the backend-a.
USER node
